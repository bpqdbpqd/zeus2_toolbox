...
FLAT_HEADER = "skychop"  # will search in DATA_DIR for the files with the header
...
while True:
    ...
    if len(new_file_set) > 0:
        for fname in new_file_set:
            if (FLAT_HEADER in fname) and (fname[-4:] == ".run"):
                flat_header = fname[:-4]
                if flat_header not in queue_list:
                    queue_list.append(flat_header)
                    flat_path = os.path.join(fd, flat_header)
                    if os.path.isfile(flat_path):
                        queue_size.append(os.path.getsize(flat_path))
                    else:
                        queue_size.append(0)
    ...
    if len(queue_list) > 0:
        ...
        if len(proc_list) > 0:
            time.sleep(max(5, size / 2E6))
            print("Processing in queue: %s" % proc_list)

            for flat_header in proc_list[::-1]:
                try:
                    skychop_result = z2pipl.reduce_skychop(
                            flat_header, data_dir=fd, write_dir=wfd,
                            write_suffix="", array_map=array_map, obs_log=obs_log,
                            pix_flag_list=None, parallel=False, return_ts=False,
                            return_pix_flag_list=True, table_save=TABLE_SAVE,
                    ...
                    success = False
                    print("First attempt failed on ", flat_header, ": ", err)
                    pass
                print("Finished processing %s" % flat_header)
                ...
                if not success:
                    ...
                    try:
                        ...
                        success = False
                        print("Second attempt failed on ", flat_header, ": ", err)
                        pass
